...
func debounce(ch chan zk.Event, delay time.Duration) chan zk.Event {
	...
	go func() {
		latest = <-ch
		glog.Infoln(latest)
		glog.Infoln("Got event. Delaying post")
		...
		for {
			...
			case ev := <-ch:
				...
				t.Reset(delay)
			case _ = <-t.C:
				glog.Infoln("No further debouncing. Posting event")
				debounced <- latest
			}
		}
	}()
...
func sinkSelfEvents(conn *zk.Conn, path string, sink chan<- zk.Event) chan<- bool {
	...
	go func() {
		...
		if err != nil {
			glog.Fatalf("failed to set listener on path: %s", err.Error())
		}
		for {
			select {
			case _ = <-control:
			...
			case ev := <-selfCh:
				...
				_, _, selfCh, err = conn.GetW(path)
				if err != nil {
					glog.Infof("failed to set listener on path: %s\n", err.Error())
				}
			}
		}
	}()
...
func sinkChildEvents(conn *zk.Conn, path string, sink chan<- zk.Event) chan<- bool {
	...
	go func() {
		...
		if err != nil {
			glog.Fatalln("failed to set listener on path")
		}
		for {
			select {
			case _ = <-control:
...
func ListenToZooKeeper(machines []string, path string, deb bool) (chan zk.Event, chan bool) {
	...
	return ListenToConn(c, path, deb, 0)
}
...
func zkCreateNodes(path string, nodes []string, c *zk.Conn) error {
	if len(nodes) > 0 {
		...
		fqPath := path + "/" + nodes[0]
		glog.Infof("Creating path: %v", fqPath)
...
func ListenToConn(c *zk.Conn, path string, deb bool, repDelay time.Duration) (chan zk.Event, chan bool) {
	...
	if err != nil {
		glog.Fatalf("Couldn't determine whether node %v exists in Zookeeper", path)
	}
	if !exists {
		glog.Infof("Node '%v' does not exist in Zookeeper, creating...", path)
		err := zkNodeCreateByPath(path, c)
		if err != nil {
			glog.Fatalf("Unable to create path '%v': %v", path, err)
		}
